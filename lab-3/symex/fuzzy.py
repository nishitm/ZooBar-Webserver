...
def fork_and_check(constr):   #used in Exercise-3
...
def concolic_test(testfunc, maxiter = 100, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    if verbose > 1:
      ...
      for x in range(len(cur_path_constr)):
        print cur_path_constr[x]
    ...
    for i in range(len(cur_path_constr)):
      ...
      #print cur_path_constr[:i],"<---"
      #cur_path_constr[len(cur_path_constr)-1] = sym_not(cur_path_constr[len(cur_path_constr)-1])
      b = cur_path_constr[i] #+ cur_path_constr[:i]
      q = [sym_not(b)]
      #print "q-->",q
      a = sym_and(*(q + cur_path_constr[:i]))   # <----- edited after exercise 3
      #print "a---->",a
      # print "a----------"
      # print a #"--type--",type(a)
      ...
      if a in checked:
        ...
        checked.add(a)
        # print "addeddddddddddd"
      

      (ok, model) = fork_and_check(a)
