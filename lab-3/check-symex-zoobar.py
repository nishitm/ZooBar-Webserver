...
def test_stuff():
  ...
  balance1 = sum([p.zoobars for p in pdb.query(zoobar.zoodb.Person).all()])     #same expr used below for finding 'zoobar mismatch'
  before = []
  for p in pdb.query(zoobar.zoodb.Person).all():
    before.append(p.zoobars)
  pdb.commit()

  tdb = zoobar.zoodb.transfer_setup()
  tdb.query(zoobar.zoodb.Transfer).delete()
  #before = pdb.query(zoobar.zoodb.Person).all()
  tdb.commit()
  ...
  environ['REQUEST_METHOD'] = 'GET'                                   #always start with GET
  environ['PATH_INFO'] = 'trans' + fuzzy.mk_str('path')               #path start with 'trans'
  ...
  bal = sum([p.zoobars for p in pdb.query(zoobar.zoodb.Person).all()])
  if bal != balance1:
    report_balance_mismatch()

  after = []
  for p in pdb.query(zoobar.zoodb.Person).all():
    after.append(p.zoobars)
  #k=0
  for i,j in zip(range(0,len(after)),range(0,len(before))):
    #print i.zoobars;
    if after[i] > before[j]:
      report_zoobar_theft()
    #k = k+1
