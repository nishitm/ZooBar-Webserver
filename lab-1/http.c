...
const char *http_request_headers(int fd)
{
    ...
    char envvar[512];
        ...
        warnx("in http_request_header---1");
    /* For lab 2: don't remove this line. */
        ...
        *colon = '\0';
        ...
        if (strcmp(buf, "CONTENT_TYPE") != 0 &&
         ...
         //   sprintf(envvar, "HTTP_%s", buf);
            a =snprintf(envvar,512, "HTTP_%s", buf);
            if(a<0 || a>512)
                warnx("buffer size is tooooo large");
...
void http_serve_directory(int fd, const char *pn) {
    ...
    static const char * const indices[] = {"index.html", "index.php", "index.cgi",
NULL};
    char name[1024];
    struct stat st;
    int i;
    ...
    for (i = 0; indices[i]; i++) {
        if(strlen(name) + strlen(pn) + strlen(indices[i]) >1024)
                return;
        dir_join(name, pn, indices[i]);
        if (stat(name, &st) == 0 && S_ISREG(st.st_mode)) {
                if(strlen(name) + strlen(getenv("SCRIPT_NAME")) + strlen(indices[i]) >1024)
                                return;
